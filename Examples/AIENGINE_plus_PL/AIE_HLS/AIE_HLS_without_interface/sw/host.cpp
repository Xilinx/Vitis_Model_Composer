...
int main(int argc, char* argv[]) {
    ...
    short int *ptr_in = (short int *) q.enqueueMapBuffer (buffer_in , CL_TRUE , CL_MAP_WRITE , 0, input_size_in_bytes);
    int *ptr_out = (int *) q.enqueueMapBuffer (buffer_out , CL_TRUE , CL_MAP_READ , 0, output_size_in_bytes);
    ...
    std::cout << "Launching the kernels...\n";

    //Launch the Kernel
    q.enqueueTask(krnl_mm2s);
    q.enqueueTask(krnl_s2mm);

    // The result of the previous kernel execution will need to be retrieved in
    // order to view the results. This call will transfer the data from FPGA to
    // source_results vector
    std::cout << "Transferring output data from the device...\n";

    q.enqueueMigrateMemObjects({buffer_out},CL_MIGRATE_MEM_OBJECT_HOST);

    std::cout << "Waiting for kernels to finish execution...\n";
    q.finish();
    ...
    std::cout << "Output validation is done...\n";
