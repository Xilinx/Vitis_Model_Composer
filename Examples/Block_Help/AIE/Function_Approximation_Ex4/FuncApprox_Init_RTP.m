...
block_size = 8; % When data type is int16, block_size = 8 for AIE-ML (16 for AIE-MLv2) 
repeat_factor = 2; 
num_blocks = length(LUTValues) / block_size;

blocks = reshape(LUTValues, block_size, num_blocks).';
blocks_repeated = repelem(blocks, repeat_factor, 1);
LUTValues_repeated = reshape(blocks_repeated.', 1, []);