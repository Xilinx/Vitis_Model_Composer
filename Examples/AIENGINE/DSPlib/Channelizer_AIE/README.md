# Polyphase Channelizer – AI Engine (AIE) Implementation

## Table of Contents
1. [Introduction](#introduction)
2. [General Channelizer Overview](#general-channelizer-overview)
3. [AIE Implementation Details](#aie-implementation-details)

## Introduction
This document summarizes the AI Engine (AIE) implementation of a Polyphase Channelizer that achieves an overall throughput of greater than 1.4 GSPS.
...
- Two reference models are created for design debug, verification and validation: 
    ...
    2. Simulink *fine-grain* integer detailed model with coefficient quantization per channel. This enables direct comparison to implementation for debug and validation.
...
## AIE Implementation Details

![](images/flow_aie.png)

The AI Engine design consists of SSR=4 paths. Each path consists of:
* **FIR TDM** (from the Vitis AI Engine DSP Library)
* **LIFO buffer** to reverse the vector sample order (see [Vector Flip Implementation Example](#vector-flip-implementation-example) below)
* **IDFT** (from the Vitis AI Engine DSP Library)

Cycle-approximate AIE simulation shows us that each path achieves a minimum of 462 MSPS, leading to an overall design throughput greater than 1.8 GSPS.

![](images/throughput_aie.png)

The design uses 24 AI Engine kernels:

![](images/resources_aie.png)

By default, each LIFO buffer's AIE Kernel has a runtime ratio of 0.9, meaning each kernel will occupy its own AI Engine tile. To save resources, we can change the LIFO kernels' runtime ratio to 0.1. The AIE compiler will then allocate the kernels so they are shared on the same tile. This reduces the throughput to 351 MSPS * 4 = 1.4 GSPS, but the design only uses 20 AI Engine tiles. 

![](images/throughput2_aie.png)

![](images/resources2_aie.png)

### Vector Flip Implementation Example

In addition to reversing order (LIFO), **manual row swapping** is required on LIFO output for SSR designs prior to the IDFT. Below shows how the combination of LIFO kernels and swapping the SSR channels affects the input to the IDFT.
