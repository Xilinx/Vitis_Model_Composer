...
(
    ...
    //*symw = *inputw;
    window_copy<cint16>(symw,inputw);
    ...
    lbuff = upd_w(lbuff, 0, vdata);   //|0:7|X|---|X|X| ->8

    window_readincr(symw, vdata);
    rbuff = upd_w(rbuff, 0, vdata);   //|0:7|X|---|24:31|X| ->32
        ...
        chess_prepare_for_pipelining
    {
        ...
        window_readincr(inputw, vdata);
        lbuff = upd_w(lbuff, 1, vdata); //|0:7|8:15|---|24:31|X| l->16
        // lbuff = upd_w(lbuff, 1, window_readincr_v8(inputw));
        window_read(symw, vdata);
        rbuff = upd_w(rbuff, 1, vdata);   window_decr_v8(symw,2); //|0:7|8:15|---|24:31|32:39| r->16

        acc0 = mul4_sym(        lbuff, 6,0x6420,2, rbuff,8,     coe,0,0x0000,1);   //d6..14 d30-38 c0..1

        window_readincr(inputw, vdata);
        lbuff = upd_w(lbuff, 0, vdata); //|16:23|8:15|---|24:31|32:39| l->24

        acc0 = mac4_sym(   acc0,lbuff,10,0x6420,2, rbuff,4,     coe,4,0x0000,1);

        window_readincr(inputw, vdata);
        lbuff = upd_w(lbuff, 1, vdata); //|16:23|24:31|---|24:31|32:39| l->32
        window_read(symw, vdata);
        rbuff = upd_w(rbuff, 1, vdata); window_incr_v8(symw,2); //|16:23|24:31|---|24:31|16:23| r->32
        ...
        lbuff = upd_w(lbuff, 0, vdata); window_decr_v8(inputw,2);
        window_readincr(symw, vdata);
        rbuff = upd_w(rbuff, 1, vdata);

        acc0 = mul4_sym_ct(     lbuff, 6,0x6420,2, rbuff,8, 5, coe,8,0x0000,1);

        // window_readdecr(inputw, vdata);
        window_read(inputw, vdata);
        lbuff = upd_w(lbuff, 0, vdata); window_decr_v8(inputw, 1);
        window_readincr(symw, vdata);
        rbuff = upd_w(rbuff, 0, vdata);

        acc0 = mac4_sym(   acc0,lbuff,18,0x6420,2, rbuff,12,    coe,4,0x0000,1);

        window_read(inputw, vdata);
        lbuff = upd_w(lbuff, 1, vdata); window_incr_v8(inputw,2);
