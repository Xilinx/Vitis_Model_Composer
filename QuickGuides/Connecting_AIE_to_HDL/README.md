...
* Set the _Ouptut Sample Time_ to **T**. Note that the bit rate into the block is **P**x**S**x(Input bit width) and the output bit rate of the block is **T**x**W**. For the internal buffers of the block not to overflow, the input rate should be less than or equal to the output rate. However, the HDL design has an initialization interval of **ii**. As such, _input rate_ <= _output rate_ x ii. 

In summary,

<img src="https://render.githubusercontent.com/render/math?math=\LARGE P \leq  \frac{T\times W\times ii}{S\times \text{(input bit width)}}">
