...
namespace xf { namespace dsp { namespace aie { namespace blas { namespace matrix_mult {
  ...
  constexpr unsigned int makeShuffleOffsets(unsigned M, unsigned N, unsigned vectorSize, unsigned leadingDim, bool hiLo)//, unsigned int loadSize) { 
  {
      unsigned int loadSize = vectorSize == 8 ? 2 : 4;
      unsigned long int ret = 0;
      unsigned mIncr = (leadingDim == ROW_MAJOR) ? 
          (loadSize < N) ? (N/loadSize)*loadSize : loadSize
          :  1; // transpose
      // We first load enough for M, then we load for N when we're tranpose. 
      unsigned nIncr = (leadingDim == ROW_MAJOR) ? 
          1 
          : (loadSize < M) ? (M/loadSize)*loadSize : loadSize; // tranpose
      for (unsigned int tileIndex=0; tileIndex<vectorSize/(M*N); ++tileIndex ){
        ...
        const unsigned tileStartIndex = (leadingDim == ROW_MAJOR) ? 
            (
               (tileIndex * N) % loadSize +
              ((tileIndex * N) / loadSize) * (M * loadSize)
            ) : (
  ...
  constexpr loHi getShuffleOffsetsInt16(const unsigned M, const unsigned N, const unsigned vectorSize, const unsigned leadingDim){
    ...
    unsigned int offLo = 
        (makeShuffleOffsets(M, N/2, vectorSize/2, leadingDim, false) & andMask) | orMask;
    unsigned int offHi = 
        (makeShuffleOffsets(M, N/2, vectorSize/2, leadingDim, true)  & andMask) | orMask;
    ...
    loHi ret = {
      .lo = offLo,
      .hi = offHi, 
      .square = (leadingDim == ROW_MAJOR) ? (unsigned) 0x3210 : (unsigned) 0x3120, // Need permute to grab sample from different 32b chunk
    };
    return ret;
  }   
template<unsigned M, unsigned N, unsigned inRow, unsigned inCol, unsigned leadingDim, typename T_D >
  static void doTile(T_D* inPtr, T_D* outPtr){
    ...
    static_assert(vectorSize <= (1024/8)/sizeof(T_D), "ERROR: calculated vector size too large for vector register.");
    static_assert(!(N==4 && leadingDim==COL_MAJOR && std::is_same_v<T_D, int16>), "ERROR: Tiling is not supported for column major int16 matrix.");
    ...
    const loHi offsets =
      std::is_same_v<T_D,int16> ? getShuffleOffsetsInt16(M,N,vectorSize, leadingDim) : getShuffleOffsets(M,N,vectorSize, leadingDim);
    ...
    const unsigned columnsPerVector = (leadingDim == ROW_MAJOR) ? 
        (vectorSize/M <= inCol) ? 
          loadSize * (loadsPerVector)/M : 
          inCol:
        (loadSize >= M ) ? 
          (loadsPerVector) : 
          (loadsPerVector) / (M / loadSize);
    const unsigned rowsPerVector = (leadingDim == ROW_MAJOR) ? 
        (vectorSize/M <= inCol) ? 
          M : vectorSize/inCol 
        : (loadSize >= M ) ? loadSize : M;
    ...
    static_assert(inRow*inCol >= vectorSize, "ERROR: Matrix is too small to implement tiling. A single matrix must consume at least 512 bits of memory.");
    ...
    for (unsigned rowPos=0; rowPos< vectorsPerCol; ++rowPos )
        chess_loop_count((vectorsPerCol)) 
        chess_prepare_for_pipelining 
    {
      ...
      for (unsigned colPos=0; colPos<vectorsPerRow; ++colPos)
          chess_loop_count(vectorsPerRow) 
          chess_prepare_for_pipelining 
      {
        ...
        for (unsigned i=0; i<(loadsPerVector); ++i)
        {
          ...
          const unsigned loadPos = (leadingDim == ROW_MAJOR) ? 
              (loadSize >= inCol) ? 
                i*loadSize : 
          ...
          //printf("loadPos=%d, pointerLoc=%d\n",loadPos, pointerLoc );
              ...
              //(largeTileIndex*loadSize + ((AChunk - largeTileIndex*largeTile)*inRow)+((i%M)*inCol + (i/M)*loadSize));
        ...
        if constexpr (N < minGranularity || leadingDim == COL_MAJOR) {
        ...
        const unsigned resultIndexBase = vectorSize*(vectorsPerRow)*rowPos+(vectorSize/rowTilesPerVector)*colPos;
        outI += vectorSize;
        ...
        //printf("\toffX[%d]*(n[%d]/incrX[%d] mod wrapX[%d]) = %d\n", offsetX, AChunk/N, incrX, wrapX, offsetX*((AChunk/N)/incrX % wrapX) );
        //printf("\toffY[%d]*(n[%d]/incrY[%d] mod wrapY[%d]) = %d\n", offsetY, AChunk/N, incrY, wrapY, offsetY*((AChunk/N)/incrY % wrapY) );
        ...
        for (unsigned tile = 0; tile < rowTilesPerVector ; ++tile) {
          ...
          aie::store_v(outPtr + resultIndexPos, mychunk.template extract<vectorSize/rowTilesPerVector>(tile));
        }
      }
    }
    //printf("\n");
...
/////
  ...
  namespace aie = ::aie; 
  template<unsigned M, unsigned N, unsigned inRow, unsigned inCol, unsigned leadingDim, typename T_D >
  void tilerKernelClass<M,N,inRow, inCol, leadingDim, T_D>::tile(input_window<T_D>* inWindow, output_window<T_D>* restrict outWindow) { 
