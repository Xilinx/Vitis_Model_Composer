...
namespace xf { namespace dsp { namespace aie { namespace blas { namespace matrix_mult {
  ...
  constexpr unsigned int makeUnTileShuffleOffsets(unsigned M, unsigned N, unsigned vectorSize, unsigned leadingDim, bool hiLo) {//, unsigned int loadSize) { 
    ...
    const unsigned tileLaneIncr = (leadingDim == ROW_MAJOR) ? N : tileSize; // if rowMajor just continue N  across tiles, otherwise treat each tile indendantly. 
    const unsigned mLaneIncr    = (leadingDim == ROW_MAJOR) ? N*tilesPerVector : 1; // if row major, we're trying to get as many columns contiguous so rows are furthest apart. if col_major, rows are stored contiguously
  ...
  constexpr loHi getUnTileShuffleOffsets(unsigned M, unsigned N, unsigned vectorSize, unsigned leadingDim){
    ...
    loHi ret = {
      .lo = makeUnTileShuffleOffsets(M, N, vectorSize, leadingDim, false) ,
  ...
  static void doUnTile(T_D* restrict inPtr, T_D* outPtr){
    ...
    static_assert(!(leadingDim==COL_MAJOR && std::is_same_v<T_D, int16>), "UnTiling is not supported for int16 matrix if matrix is column major.");
    const unsigned largeTile = (M*N) * (inCol/N);
    //printf("LargeTileSize=%d\n",largeTile);
    loHi offsets =  
        std::is_same_v<T_D,int16> ? getUnTileShuffleOffsetsInt16(M,N,vectorSize, leadingDim) : 
    ...
    const unsigned colsPerLoad = (loadSize <= N) ? loadSize : N * std::max((loadSize / (M * N)), (unsigned) 1); // deal with edge case where can load multiple full tiles with a single load
    const unsigned rowsPerLoad = (loadSize <= N) ? 1        : (((loadSize / N) - 1 ) % M ) + 1;
    // We move pointers between loads to optimise for col/row major. 
    const unsigned colsPerVector = (loadSize <= N && leadingDim == ROW_MAJOR) ? 
      (vectorSize <= inCol) ? vectorSize : inCol :
      tilesPerVector * N;
    const unsigned rowsPerVector = (loadSize <= N && leadingDim == ROW_MAJOR) ? 
      (vectorSize <= inCol) ? 1 : vectorSize/inCol :
      M; // todo? 

    const unsigned vectorsPerCol = inRow/rowsPerVector; 
    const unsigned vectorsPerRow = inCol/colsPerVector; 
    //printf("colsPerLoad: %d, rowsPerLoad: %d, colsPerVector: %d, rowsPerVector: %d, vectorsPerCol: %d, vectorsPerRow: %d\n",colsPerLoad, rowsPerLoad, colsPerVector, rowsPerVector, vectorsPerCol, vectorsPerRow );
    ...
    for (unsigned outerDimIdx = 0; outerDimIdx < outerLoopCount; ++outerDimIdx) 
        chess_loop_count((outerLoopCount)) 
        chess_prepare_for_pipelining 
    { 
      const unsigned ptrOuterBase = (leadingDim == ROW_MAJOR) ? 
          (rowsPerVector < M) ? 
            (((outerDimIdx*rowsPerVector) % M) * N) + (((outerDimIdx*rowsPerVector) / M) * (inCol * M)) : 
            outerDimIdx * outerLoopIncr : 
          outerDimIdx * outerLoopIncr;
      for (unsigned innerDimIdx = 0; innerDimIdx < innerLoopCount; ++innerDimIdx)
        chess_loop_count((innerLoopCount)) 
        chess_prepare_for_pipelining 
      { 
        const unsigned ptrInnerBase = innerDimIdx * innerLoopIncr; 
        //printf("outerDimIdx: %d, ptrOuterBase: %d,  innerDimIdx: %d, ptrInnerBase: %d\n",outerDimIdx,  ptrOuterBase, innerDimIdx, ptrInnerBase);
        
        aie::vector<T_D, vectorSize> vec;
        
        #pragma unroll ((loadsPerVector))
        for (unsigned loadIdx = 0; loadIdx < loadsPerVector; ++loadIdx) { 
          const unsigned innerLoadPtr = (leadingDim == ROW_MAJOR) ? 
              (loadSize <= N ) ?
                  loadSize * (loadIdx % ((N/loadSize)*rowsPerVector)) + (loadIdx / ((N / loadSize)*rowsPerVector)) * M * N  : // skip over the rows; only load columns within a single row. 
                  loadSize * loadIdx
            : // colMajor
              (loadSize <= N*M) ?
                //potenially could do something different for loadSize<N
                // previously was inCol * M for outerTile increment - changed to go to the next tile out of simplicity. 
                loadSize * (loadIdx % ((M * N) / loadSize)) + (loadIdx / ((M * N) / loadSize)) * (M * N) : // within one tile, just load the whole tile. outwith that tile, move to the next tile. - This could be simplied to just loadSize * idx
        ...
        for (unsigned outerStoreIdx = 0; outerStoreIdx < outerDimPerVector; ++outerStoreIdx) { 
          ...
          for (unsigned innerStoreIdx = 0; innerStoreIdx < std::max(innerDimPerVector/storeSize, (unsigned) 1); ++innerStoreIdx) { 
            ...
            const unsigned sliceIdx = (!shuffleIsNeeded && outerDimPerVector > 1) ? 
                innerStoreIdx * outerDimPerVector + outerStoreIdx : 
                innerStoreIdx                     + outerStoreIdx*(innerDimPerVector/storeSize); 
            const unsigned storePtr = 
              innerStoreIdx*storeSize + 
              storeOuterPtr + 
...
////
  ...
  namespace aie = ::aie; 
  template<unsigned M, unsigned N, unsigned inRow, unsigned inCol, unsigned leadingDim, typename T_D >
  void untilerKernelClass<M,N,inRow, inCol, leadingDim, T_D>::unTile(input_window<T_D>* inWindow, output_window<T_D>* restrict outWindow) { 
